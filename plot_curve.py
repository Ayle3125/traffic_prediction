...
def plot_curve(keys, inputfile, outputfile, format='png',
    ...
    for line in inputfile:
        ...
        if found:
            data_all = compiled_cost_pattern.findall(line)
            
            data.append([data_cnt,numpy.mean(  map(float, data_all) )])
            data_cnt = data_cnt+1
            #data.append([float(x) for x in found.groups()])
        if found_test:
            data_all = compiled_cost_pattern.findall(line)
            test_data.append([test_data_cnt,numpy.mean(  map(float, data_all) )])
